...
#' @param K Integer (scalar or vector) specifying the number of clustering to consider (default is 2).
...
#'   }
#' @param initialization A character string or a list to specify the in initial values for the EM algorithm:
#'  \itemize{
#'   \item{'GNN': uses a graphical neural network approach to generate initial values (default)}
...
#'                                 \item{\code{'prob_mat'}: using change in the absolute difference in \eqn{Z} (i.e., the \eqn{N \times K} cluster assignment probability matrix) between subsequent iterations (default)}
#'                                 \item{\code{'Q'}: using change in the absolute difference in the objective function of the E-step evaluated using parameters from subsequent iterations}
...
#' \item{\code{tolerance_CER}}{A numeric specifying the tolerance used for \code{termination_rule = 'ARI'} (default is \code{0.01}).}    
...
#' \item{\code{quantile_diff}}{An numeric in \code{[0,1]} specifying the quantile used in computing the change in the absolute difference of \eqn{Z} and \eqn{U} between subsequent iterations (default is \code{1}, i.e., max).}
#' \item{\code{beta_temp_schedule}}{A numeric vector specifying the temperature schedule for deterministic annealing (default is \code{1}, i.e., deterministic annealing no utilized).}       
...
#'                                 }}
#' \item{\code{sd_random_U_GNN}}{(only relevant when \code{initialization = 'GNN'}) A positive numeric value specifying the standard deviation for the random draws from a normal distribution (default is \code{1}).} 
